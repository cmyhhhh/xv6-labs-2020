...
iappend(uint inum, void *xp, int n)
{
  ...
  while(n > 0){
    ...
    if(fbn < NDIRECT){
      ...
      rsect(xint(indirect[(fbn - NDIRECT - NINDIRECT) / NINDIRECT]), (char*)indirect2);
      if(indirect2[(fbn - NDIRECT - NINDIRECT) % NINDIRECT] == 0){
        indirect2[(fbn - NDIRECT - NINDIRECT) % NINDIRECT] = xint(freeblock++);
        wsect(xint(indirect[(fbn - NDIRECT - NINDIRECT) / NINDIRECT]), (char*)indirect2);
      }
      x = xint(indirect2[(fbn - NDIRECT - NINDIRECT) % NINDIRECT]);
    }
